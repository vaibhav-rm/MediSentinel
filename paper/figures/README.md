...
paper's Tables I & II and the Results section.
...
| `fig4_detection_performance.png` | Results → *Detection Performance* (with Table I) | **Fig. 4.** Detection accuracy, F1, and false-positive rate per agent and dataset. All agents meet or approach the 95% accuracy target with FPR < 2.5%. |
| `fig5_adversarial_robustness.png` | Results → *Adversarial Robustness* (with Table II) | **Fig. 5.** Network Monitor accuracy under FGSM, PGD, and Carlini–Wagner attacks before and after IBM ART adversarial hardening. Hardening recovers accuracy from as low as 38% to 79–90%. |
